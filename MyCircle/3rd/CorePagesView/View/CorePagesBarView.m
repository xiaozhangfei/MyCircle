...
-(void)btnsPrepare{
    ...
    _bottomLineView.backgroundColor = HexColor(0xDEDEDE);
...
-(void)layoutSubviews{
    ...
    [self.subviews enumerateObjectsUsingBlock:^(UIView *subView, NSUInteger idx, BOOL *stop) {
        ...
        if([subView isKindOfClass:[CorePagesBarBtn class]]){//过滤其他控件
            ...
            if(i==_page && !_hasSelectedFirstBtn){
    ...
    _bottomLineView.frame = CGRectMake(0, self.frame.size.height - 2, self.frame.size.width, 2);
}

- (void)refreshBarViewOnceToken {
...
-(UIView *)lineView{
    ...
    if(!_lineView){
        ...
        _lineView.backgroundColor = HexColor(0x51B9C7);
